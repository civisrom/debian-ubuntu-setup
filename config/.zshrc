...
if [[ -r "$ZSH/oh-my-zsh.sh" ]]; then
    ...
    echo "Oh My Zsh не найден: $ZSH/oh-my-zsh.sh" >&2
fi
...
# --- sudo + алиасы -----------------------------------------------------------
# Пробел в конце позволяет zsh разворачивать все алиасы после sudo.
# Без этого sudo видит только реальные команды, но не zsh-алиасы.
alias sudo='sudo '
...
# --- Git ---------------------------------------------------------------------
alias g='git'                                      # короткий вызов git
alias gs='git status --short --branch'             # компактный статус + текущая ветка
alias gst='git status'                             # полный статус репозитория
alias ga='git add'                                 # добавить файл(ы) в индекс: ga file
alias gaa='git add --all'                          # добавить все изменения в индекс
alias gap='git add --patch'                        # интерактивно выбрать части изменений
alias gc='git commit'                              # создать коммит через редактор
alias gcm='git commit -m'                          # создать коммит с сообщением: gcm "msg"
alias gca='git commit --amend'                     # изменить последний коммит
alias gcan='git commit --amend --no-edit'          # добавить изменения в последний коммит без смены сообщения
alias gd='git diff'                                # показать unstaged diff
alias gds='git diff --staged'                      # показать diff того, что уже в индексе
alias gdc='git diff --cached'                      # алиас к staged diff
alias gl='git log --oneline --decorate --graph --all'       # компактная история всех веток графом
alias gla='git log --oneline --decorate --graph --all --stat'  # история всех веток + список изменённых файлов
alias gb='git branch'                              # список локальных веток
alias gba='git branch --all'                       # список локальных и удалённых веток
alias gsw='git switch'                             # переключиться на ветку: gsw main
alias gcb='git switch -c'                          # создать и переключиться на новую ветку
alias gco='git checkout'                           # старый checkout для файлов/веток
alias gpl='git pull --ff-only'                     # подтянуть изменения без merge-коммита
alias gp='git push'                                # отправить текущую ветку
alias gpu='git push -u origin HEAD'                # запушить новую ветку и привязать upstream
alias gpf='git push --force-with-lease'            # безопасный force-push с проверкой удалённой ветки
alias gf='git fetch --all --prune'                 # обновить все remotes и удалить stale refs
alias gr='git remote -v'                           # показать remotes
alias grb='git rebase'                             # выполнить rebase
alias grbc='git rebase --continue'                 # продолжить rebase после решения конфликтов
alias grba='git rebase --abort'                    # отменить текущий rebase
alias gm='git merge'                               # слить ветку
alias gcp='git cherry-pick'                        # применить отдельный коммит
alias gcpa='git cherry-pick --abort'               # отменить текущий cherry-pick
alias gstash='git stash push -u'                   # спрятать tracked + untracked изменения
alias gstashp='git stash pop'                      # вернуть последний stash и удалить его из списка
alias gstashl='git stash list'                     # список stash-записей
alias gtags='git tag --sort=-creatordate'          # теги от новых к старым
alias gundo='git restore'                          # откатить изменения в рабочем дереве: gundo file
alias gundos='git restore --staged'                # убрать файл из индекса, не теряя изменения
alias gwip='git add --all && git commit -m "wip"'  # быстрый временный WIP-коммит

unalias gcd gignore 2>/dev/null                    # освободить имена, если их занял Oh My Zsh git plugin
...
gignore() {
    (( $# )) || { echo "Usage: gignore <template> [template...]"; return 1; }
    local templates="${(j:,:)@}"
    curl -fsSL "https://www.toptal.com/developers/gitignore/api/${templates}"
}
...
unalias dstop drestart 2>/dev/null
dstop() {
    local containers
    containers=("${(@f)$(docker ps -q 2>/dev/null)}")
    (( ${#containers[@]} )) && docker stop "${containers[@]}" || echo "Нет запущенных контейнеров"
}
drestart() {
    local containers
    containers=("${(@f)$(docker ps -q 2>/dev/null)}")
    (( ${#containers[@]} )) && docker restart "${containers[@]}" || echo "Нет запущенных контейнеров"
}
#
...
# System aliases
alias ll='ls -lah'                                # подробный список файлов, включая скрытые
alias ports='ss -tulnp'                           # все слушающие TCP/UDP порты с процессами
alias mem='free -h'                               # использование RAM в человекочитаемом виде
alias df='df -h'                                  # свободное место на дисках
